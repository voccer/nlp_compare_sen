#python cnn_old_vn.py --trainfile ../data-vn/vn-train --testfile ../data-vn/vn-val --predfile ../data-vn/pred/pred-vn-cnn-old-6.txt --v 1
import pprint
import argparse
import torch
...
#             baseline_features = np.zeros(shape = (512, 11))
            ...
            optimizer.zero_grad()
            output = cnn(s1, s2, baseline_features).reshape(-1)
            loss = criterion(output, score)
            loss.backward()
            optimizer.step()
            if (i) % 5 == 0:
                print(datetime.datetime.now(), 'Epoch {} batch {} loss: {}' .format(epoch, i, loss.data))
                f.write('Epoch {} batch {} loss: {} \n' .format(epoch, i, loss.data))
                
        torch.save(cnn.state_dict(), "./data/model/cnn-vn-{}.mdl".format(epoch))
        cnn.eval()
        res = []
        cnt = 0
        loss_val = 0
...
#             baseline_features = np.zeros(shape = (512, 11))
            ...
            res.append(output.data.cpu().numpy()[0])
        print('test error: ', float(loss_val) / len(data_test))
        f.write('test error: {}\n'.format(float(loss_val) / len(data_test)))
        f.close()
        
        
        
#     torch.save(cnn.state_dict(), "./data/cnn-vn-2.mdl")
...
if __name__ == '__main__':
    ...
    print('Time to end: {}s'.format(end_time -start_time))